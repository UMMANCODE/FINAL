  ...
  : IUserHouseService
{
  ...
  public async Task<BaseResponse> GetPaginated(int pageNumber = 1, int pageSize = 1)
  {
    ...
    var data = new PaginatedList<UserHouseGetAllDto>(mapper.Map<List<UserHouseGetAllDto>>(paginated.Items),
      paginated.TotalPages, pageNumber, pageSize);
  ...
  {
    var house = await houseRepository.GetAsync(x => x.Id == id && !x.IsDeleted, "HouseImages", "Comments", "Bids",
      "Discounts");
  ...
  public async Task<BaseResponse> Update(int id, UserHouseUpdateDto updateDto)
  {
    ...
    try
    {
      ...
      var allImages = new List<HouseImage>(house.HouseImages.Where(image =>
        updateDto.IdsToDelete != null && !updateDto.IdsToDelete.Contains(image.Id)));
      var deletedImages = house.HouseImages
        .Where(image => updateDto.IdsToDelete != null && updateDto.IdsToDelete.Contains(image.Id)).ToList();
  ...
  public async Task<BaseResponse> Filter(PropertyStatus? status = null, PropertyType? type = null,
    PropertyState? state = null)
  {
    var houses = await houseRepository.GetAllAsync(x => !x.IsDeleted);

    var filteredHouses = houses.Where(x =>
        (status == null || x.Status == status) && (type == null || x.Type == type) &&
